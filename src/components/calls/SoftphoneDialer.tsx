...
export default function SoftphoneDialer({
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'voip_providers'), (snapshot) => {
      if (!snapshot.empty) {
        const loaded: VoipProvider[] = snapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        } as VoipProvider));
        setProviders(loaded);
  ...
  return (
    <div className={`relative bg-slate-950 text-white rounded-3xl border border-slate-800 shadow-2xl p-5 sm:p-7 ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-slate-900">
        ...
        {/* Estado del Servidor SIP / Carrier */}
      ...
      <div className="mt-5 space-y-4">
        {/* Selector de Carrier de Voz */}
        <div className="flex items-center justify-between gap-3">
          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
            <Radio className="w-3 h-3 text-cyan-400" />
            <span>{t('voip.carrier', 'Carrier de Voz')}:</span>
          </label>
          <select
            value={selectedCarrier}
            onChange={(e) => setSelectedCarrier(e.target.value)}
            disabled={callStatus !== 'idle'}
            className="bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-200 font-medium focus:outline-none focus:border-cyan-400 transition-colors disabled:opacity-50"
          >
            {providers.map(p => (
